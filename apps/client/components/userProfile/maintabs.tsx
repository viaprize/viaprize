...
export default function MainTabsUserProfile() {
  ...
  const getPrizesOfUserMutation = useQuery(['getPrizesOfUser', undefined], async () => {
    const prizes = await ((await backendApi()).users.usernamePrizesDetail(query.id as string));

    const prizesWithBalancePromise = prizes.data.map(
      async (prize) => {
  ...
  const getSubmissionsOfUserMutation = useQuery(['getSubmissionsOfUser', undefined], async () => {
        ...
        <Skeleton visible={getPrizesOfUserMutation.isLoading}>
          {
            getPrizesOfUserMutation.data?.map(prize =>
              <ExploreCard description={prize.description}
