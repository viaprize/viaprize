...
import { Kickstarter } from "../../typechain-types/proxyportals/kickstarter.sol";
...
describe("initialize", () => {
    it("should correctly initialize the contract", async () => {
        ...
        await kickstarter.initialize(proposers, admins, goal, deadline, allowDonationAboveGoalAmount, platformFee);
        ...
        expect(await kickstarter.allowDonationAboveGoalAmount()).to.equal(allowDonationAboveGoalAmount);
...
describe("addFunds", () => {
    it("should revert if no funds are sent", async () => {
        await expect(kickstarter.addFunds({ value: 0 })).to.be.revertedWith("NotEnoughFunds");
    });

    it("should revert if campaign is ended", async () => {
        await kickstarter.endEarlyandRefund();
        await expect(kickstarter.addFunds({ value: 100 })).to.be.revertedWith("FundingToContractEnded");
    });

    it("should correctly add funds", async () => {
        await kickstarter.addFunds({ value: 100 });

        expect(await kickstarter.patronAmount((await ethers.provider.getSigner()).getAddress())).to.equal(100);
...
describe("endEarlyandRefund", () => {
    it("should revert if not called by a proposer or admin", async () => {
        await expect(kickstarter.connect(await ethers.getSigner("0x1f00DD750aD3A6463F174eD7d63ebE1a7a930d0c")).endEarlyandRefund()).to.be.revertedWith("You are not a proposer or admin.");
    });
...
describe("endKickStarterCampaign", () => {
    it("should revert if not called by a proposer or admin", async () => {
        await expect(kickstarter.connect(await ethers.getSigner("0x1f00DD750aD3A6463F174eD7d63ebE1a7a930d0c")).endKickStarterCampaign()).to.be.revertedWith("You are not a proposer or admin");
    }
    )
    it("should correctly end the campaign", async () => {
