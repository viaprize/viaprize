...
import type { Gofundme } from '../../typechain-types/proxyportals/gofundme.sol'
...
describe('addFunds', () => {
  it('should revert if no funds are sent', async () => {
    await expect(gofundme.addFunds({ value: 0 })).to.be.revertedWith(
      'NotEnoughFunds',
    )
  })
  ...
  it('should revert if campaign is ended', async () => {
    ...
    await expect(gofundme.addFunds({ value: 100 })).to.be.revertedWith(
